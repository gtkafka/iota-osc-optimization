...
def plot_bunch_lengths(input,dl,sE,sP,sig2,plot_of):#,t511,t512,t516,t522,t526,t566):
    ...
    for k in range(len(lengths)):
        ...
        histo=diff
        x,rms=get_rms(histo,cut='no')
        diff_rms.append(rms)
        if plot_of=='real':
            histo=lengths[k]
            x,rms=get_rms(bunch=lengths[k],cut='no')
            real_rms.append(rms)
    ...
    if histo==diff:
        ...
        #plt.plot(s,sE,'-',color='green')
        #plt.plot(s,sP,'-',color='black')
        tds=str(dl[45]*1e6)

    else:
        plt.plot(z,real_rms,'-o')
    plt.title('RMS VS. Z'+'  '+'Theoretical lengthening: '+tds+'um')
...
_s,_betx,_alfx,_gamx =  get_m56.get_values('ptc_abc',1,2,6,13)
#print _betx[0], _alfx[0]
_s,_M51,_M52,_M56    =  get_m56.get_values('ptc_rmatrix',1,2,3,4)


s,m51,m52,m56,m56p   =  get_m56.get_M5s(_s,_M51,_M52,_M56)

sigE,sigP,tot_ds,m56label     =  get_m56.get_bunch_lengths(_betx,_alfx,_gamx,_M51,_M52,_M56,ex=(3.98e-9)/2,sigP=0.00011)
sigma2=get_m56.get_T5s()
# #####READ IN TRACKONE FROM MADX BYPASS OUTPUT######
n_obsv=11
beam=read_file('../realtrackone', nparts=10000,obsv=n_obsv)
tot_ds= tot_ds[0:100]
s = s[0:100]
sigP= sigP[0:100]
sigE= sigE[0:100]
plot_bunch_lengths(beam,tot_ds,sigE,sigP,sigma2,plot_of='diff')#,t511=t11,t512=t12,t516=t16,t522=t22,t526=t26,t566=t56)
